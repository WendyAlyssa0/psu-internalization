...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_create'])) {
    ...
    if (strtolower($role) === 'applicant' && $program === '') {
    ...
    if (!$createErrors) {
        ...
        if ($check->fetch()) {
            ...
            try {
                $pdo->prepare("
                    INSERT INTO users
                        (first_name, middle_name, last_name, extension_name,
                         birthdate, program, email, user_role, status, password_hash)
                    VALUES (?,?,?,?,?,?,?,?,?,?)
                ")->execute([
                    $firstName,
                    $middleName  ?: null,
                ...
                $createErrors[] = 'Database error: ' . $e->getMessage();
            }
        }
    }
}
        ...
        <div class="modal-body">
            ...
            <?php if ($createSuccess): ?>
                <div class="success-box">
                    ...
                    <div style="margin-top:.5rem"><strong>Email:</strong> <code><?= h($createdEmail) ?></code></div>
                    <div><strong>Temporary Password:</strong> <code><?= h($createdPass) ?></code></div>
                </div>
            <?php endif; ?>
            ...
            <form method="POST" id="createUserForm">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group col-5">
                        ...
                        <input type="text" name="first_name" placeholder="e.g., Sherwin"
                    ...
                    <div class="form-group col-3">
                        ...
                        <select name="extension_name">
                            ...
                            <?php foreach (['Jr.','Sr.','I','II','III','IV'] as $ext): ?>
                                <option <?= (($_POST['extension_name'] ?? '') === $ext) ? 'selected' : '' ?>>
                                    <?= h($ext) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group col-5">
                        ...
                        <select name="role" id="roleSelect" required>
                            ...
                            <?php foreach (['super admin' => 'Super Admin', 'admin' => 'Admin', 'applicant' => 'Applicant'] as $val => $label): ?>
                                <option value="<?= h($val) ?>"
                    ...
                    <div class="form-group col-12" id="programField" style="display:none">
                        <label>Assigned Program <span class="required">*</span></label>
                        <select name="program" id="programSelect">
                            <option value="BSIT" <?= (($_POST['program'] ?? '') === 'BSIT') ? 'selected' : '' ?>>
                                Bachelor of Science in Information Technology
                            </option>
                            <option value="BSBA" <?= (($_POST['program'] ?? '') === 'BSBA') ? 'selected' : '' ?>>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group col-6">
                        ...
                        <input type="text" name="email" id="emailPreview" readonly
                               placeholder="Generates automatically…">
                    </div>

                    <div class="form-group col-6">
                        <label>Default Password</label>
                        <input type="text" value="<?= h($createdPass) ?>" readonly>
                    </div>
...
(function () {
    ...
    function generateEmailPreview() {
        var f = document.querySelector('[name="first_name"]').value.trim();
        var l = document.querySelector('[name="last_name"]').value.trim();
    ...
    function toggleProgram() {
        ...
        var show   = role === 'applicant';
        field.style.display = show ? 'flex' : 'none';
        show ? select.setAttribute('required','required') : select.removeAttribute('required');
    }

    document.querySelector('[name="first_name"]').addEventListener('input', generateEmailPreview);
    document.querySelector('[name="last_name"]').addEventListener('input',  generateEmailPreview);
